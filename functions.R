...
# Descomposición por edad de la diferencia de e0 entre dos tablas de vida
# lx1, Lx1: tabla periodo 1
# lx2, Lx2: tabla periodo 2
desc <- function(lx1, Lx1, lx2, Lx2) {
  
  # Chequeos básicos
  m <- length(lx1)
  if (length(lx2) != m | length(Lx1) != m | length(Lx2) != m) {
    stop("Todas las entradas deben tener la misma longitud")
  }
  
  # Sobrevivientes al nacimiento en cada periodo
  l01 <- lx1[1]
  l02 <- lx2[1]
  
  # Contribuciones por edad:
  # C_x = Lx2 / l0_2 - Lx1 / l0_1
  diff <- (Lx2 / l02) - (Lx1 / l01)
